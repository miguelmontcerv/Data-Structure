...
struct arbolDeBanco *insertarArbol (struct arbolDeBanco *,
				    struct cuentaHabiente);
void mostrarArbol (struct arbolDeBanco *);
struct arbolDeBanco *buscarArbol (struct arbolDeBanco *,
				  struct cuentaHabiente persona);
...
struct arbolDeBanco *eliminarDeArbol (struct arbolDeBanco *,
				      struct cuentaHabiente);
int menu ();
struct cuentaHabiente crearCH ();
long long generarNumValido ();
struct arbolDeBanco *buscarArbol_No_Cuenta (struct arbolDeBanco *,
					    long long No);
int preguntar ();
